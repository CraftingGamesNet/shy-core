...
import { ShyDistribution, ShyModule, ShyServer } from '../../common/distribution/DistributionFactory'
import { Type } from 'shy-distribution-types'
...
export class DistributionIndexProcessor extends IndexProcessor {
    ...
    constructor(commonDir: string, protected distribution: ShyDistribution, protected serverId: string) {
        super(commonDir)
    }
    ...
    public async validate(onStageComplete: () => Promise<void>): Promise<{[category: string]: Asset[]}> {
        ...
        const server: ShyServer = this.distribution.getServerById(this.serverId)!
        if(server == null) {
    ...
    private async validateModules(modules: ShyModule[], accumulator: Asset[]): Promise<void> {
    ...
    public async loadVersionManifest<T>(modLoaderModule: ShyModule): Promise<T> {
