import { Distribution, Server, Module, Type, Required as ShyRequired, JavaVersionProps, JavaPlatformOptions, Platform, JdkDistribution } from 'shy-distribution-types'
...
export class ShyDistribution {

    private mainServerIndex!: number

    public readonly servers: ShyServer[]
    ...
    constructor(
        ...
        this.servers = this.rawDistribution.servers.map(s => new ShyServer(s, commonDir, instanceDir))
    }
    ...
    public getMainServer(): ShyServer | null {
        return this.mainServerIndex < this.servers.length ? this.servers[this.mainServerIndex] : null
    }

    public getServerById(id: string): ShyServer | null {
        return this.servers.find(s => s.rawServer.id === id) || null
    }

}

export class ShyServer {

    public readonly modules: ShyModule[]
    ...
    constructor(
        ...
        this.modules = rawServer.modules.map(m => new ShyModule(m, rawServer.id, commonDir, instanceDir))
    }
...
export class ShyModule {

    public readonly subModules: ShyModule[]

    private readonly mavenComponents: Readonly<MavenComponents>
    private readonly required: Readonly<Required<ShyRequired>>
    ...
    constructor(
        ...
        if(this.rawModule.subModules != null) {
            this.subModules = this.rawModule.subModules.map(m => new ShyModule(m, serverId, commonDir, instanceDir))
        } else {
            this.subModules = []
        }
    ...
    private resolveRequired(): Required<ShyRequired> {
    ...
    public getRequired(): Readonly<Required<ShyRequired>> {
