...
import { Distribution } from 'shy-distribution-types'
...
import { ShyDistribution } from './DistributionFactory'
...
export class DistributionAPI {
    ...
    private distribution!: ShyDistribution
    ...
    public async getDistribution(): Promise<ShyDistribution> {
        if(this.rawDistribution == null) {
            this.rawDistribution = await this.loadDistribution()
            this.distribution = new ShyDistribution(this.rawDistribution, this.commonDir, this.instanceDir)
        }
        return this.distribution
    }

    public async getDistributionLocalLoadOnly(): Promise<ShyDistribution> {
        if(this.rawDistribution == null) {
    ...
    public async refreshDistributionOrFallback(): Promise<ShyDistribution> {
        ...
        if(distro == null) {
            ...
            this.distribution = new ShyDistribution(distro, this.commonDir, this.instanceDir)
